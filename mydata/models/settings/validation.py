...
def check_for_missing_required_fields():
    ...
    if settings.general.api_key.strip() == "":
        message = (
            "Please enter your MyTardis API key.\n\n"
...
def warn_if_ignoring_invalid_user_folders():
    ...
    if not settings.advanced.upload_invalid_user_or_group_folders:
        if settings.advanced.folder_structure.startswith("User Group"):
            ...
            "User" in settings.advanced.folder_structure
            or "Email" in settings.advanced.folder_structure
        ):
...
def check_filters(set_status_message):
    ...
    if settings.filters.ignore_new_files:
        message = (
            "Files newer than %s minute(s) are being ignored."
    ...
    if settings.filters.use_includes_file and not settings.filters.use_excludes_file:
        ...
        logger.warning(message)
        log_if_test_run("WARNING: %s" % message)
    elif not settings.filters.use_includes_file and settings.filters.use_excludes_file:
        message = (
        ...
        logger.warning(message)
        log_if_test_run("WARNING: %s" % message)
    elif settings.filters.use_includes_file and settings.filters.use_excludes_file:
        message = (
            "Files matching patterns in excludes "
            "file will not be scanned for upload, "
...
def check_datafile_glob_files(set_status_message):
    ...
    if settings.filters.use_includes_file:
        ...
        if set_status_message:
            set_status_message(message)
        perform_globs_file_validation(
            settings.filters.includes_file, "Includes", "includes", "includes_file"
        )
    ...
    if settings.filters.use_excludes_file:
        ...
        if set_status_message:
            set_status_message(message)
        perform_globs_file_validation(
            settings.filters.excludes_file, "Excludes", "excludes", "excludes_file"
        )
...
def check_mytardis_url(set_status_message=None):
    ...
    try:
        ...
        if history:
            message = (
                "MyData attempted to access MyTardis at "
            ...
            message += "\n\n"
            message += (
                "A redirection could be caused by any of "
        ...
        if response.status_code == 200:
            ...
            logger.debug(
                "Received HTTP %d while trying to access "
                "MyTardis server (%s)."
                % (response.status_code, settings.general.mytardis_url)
            )
            message = (
                "Please enter a valid MyTardis URL.\n\n"
...
def check_facility(set_status_message=None):
    ...
    if settings.general.facility is None:
        facilities = Facility.get_my_facilities()
        message = (
            'Facility "%s" was not found in MyTardis.' % settings.general.facility_name
        )
        if facilities:
            message += (
            ...
            message += (
                ...
                "that facility."
                % (settings.general.facility_name, settings.general.username)
            )
        suggestion = None
        if len(facilities) == 1:
            suggestion = facilities[0].name
...
def check_contact_email_and_email_folders(set_status_message):
    ...
    if (
        ...
        and settings.advanced.validate_folder_structure
    ):
...
def perform_globs_file_validation(file_path, upper, lower, field):
    ...
    if not os.path.exists(file_path):
        message = "Specified %s file doesn't exist." % lower
        raise InvalidSettings(message, field)
    if not os.path.isfile(file_path):
        message = "Specified %s file path is not a file." % lower
        raise InvalidSettings(message, field)
...
def count_datasets_in_dirs(dirs):
    ...
    if settings.filters.ignore_old_datasets or settings.filters.ignore_new_datasets:
        dataset_count = 0
        for folder in dirs:
...
def folder_glob(level, instrument_name="*"):
    ...
    glob_dict = {
        ...
        "User Group / Dataset": [user_or_group_glob, dataset_glob],
        "Email / Dataset": [user_or_group_glob, dataset_glob],
        "Experiment / Dataset": [exp_glob, dataset_glob],
        "Username / Experiment / Dataset": [user_or_group_glob, exp_glob, dataset_glob],
        "User Group / Experiment / Dataset": [
            user_or_group_glob,
            exp_glob,
            dataset_glob,
        ],
        "Email / Experiment / Dataset": [user_or_group_glob, exp_glob, dataset_glob],
