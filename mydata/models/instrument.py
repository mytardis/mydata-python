...
class Instrument:
    ...
    def create_instrument(facility, name):
        ...
        instrument_dict = {"facility": facility.resource_uri, "name": name}
    ...
    def get_instrument(facility, name):
        ...
        num_instruments_found = instruments_dict["meta"]["total_count"]
        if num_instruments_found == 0:
            message = 'Instrument "%s" was not found in MyTardis' % name
            logger.warning(message)
            return None
        logger.debug(
            'Found instrument record for name "%s" '
    ...
    def rename_instrument(facility_name, old_name, new_name):
        ...
        old_instrument = Instrument.get_instrument(facility, old_name)
        if not old_instrument:
            raise Exception(
                "Instrument record for old instrument "
                "name not found in rename_instrument."
            )
        duplicate = Instrument.get_instrument(facility, new_name)
        if duplicate:
            raise DuplicateKey('Instrument with name "%s" ' "already exists" % new_name)
    ...
    def rename(self, name):
        ...
        :raises requests.exceptions.HTTPError:
        """
        logger.info('Renaming instrument "%s" to "%s".' % (str(self), name))
