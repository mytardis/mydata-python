    ...
    def get_exp_for_folder(folder):
        ...
        if folder.user_folder_name:
            url += "&user_folder_name=%s" \
                % urllib.parse.quote(folder.user_folder_name.encode('utf-8'))
    ...
    def create_exp_for_folder(folder):
        ...
        if FLAGS.test_run_running:
            message = "CREATING NEW EXPERIMENT FOR FOLDER: %s\n" \
                ...
                "    Owner: %s" \
                % (folder.get_rel_path(),
                   exp_title, instrument_name, user_folder_name,
                   folder.owner.username)
            logger.testrun(message)
            return None
    ...
    def log_exp_creation(folder):
        ...
        if folder.user_folder_name:
            message = "Creating experiment for instrument '%s', " \
                "user folder '%s'." % (
                    instrument_name, folder.user_folder_name)
            if folder.group_folder_name:
            ...
            message = "Creating experiment for uploader '%s', " \
                "user group folder '%s'." % (
                    instrument_name, folder.group_folder_name)
        else:
            message = \
                "Creating experiment for uploader '%s'" % instrument_name
        logger.info(message)
        return message
    ...
    def log_exp_not_found(folder):
        ...
        if folder.user_folder_name:
            message = "Experiment not found for %s, '%s'" \
                % (folder.user_folder_name, folder.experiment_title)
            if folder.group_folder_name:
                message += ", '%s'" % folder.group_folder_name
        elif folder.group_folder_name:
            message = "Experiment not found for %s, '%s'" \
                % (folder.group_folder_name, folder.experiment_title)
        else:
            message = \
                "Experiment not found for '%s'" % folder.experiment_title
        logger.debug(message)
        return message
    ...
    def log_exp_found(folder):
        ...
        if folder.user_folder_name:
            message = "Found existing experiment with title '%s' " \
                "and user folder '%s'" % (folder.experiment_title,
            ...
            message = "Found existing experiment with title '%s' " \
                "and user group folder '%s'" % (folder.experiment_title,
                                                folder.group_folder_name)
        else:
            message = "Found existing experiment with title '%s'." \
