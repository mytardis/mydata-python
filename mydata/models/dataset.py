...
class Dataset:
    ...
    def create_dataset_if_necessary(folder):
        ...
        if existing_dataset:
            if FLAGS.test_run_running:
                message = (
                    "ADDING TO EXISTING DATASET FOR FOLDER: %s\n"
                    "    URL: %s/%s\n"
                    "    Description: %s\n"
                    "    In Experiment: %s/%s"
                    % (
        ...
        exp_uri = (
            experiment.resource_uri.replace("mydata_experiment", "experiment")
            if experiment
            else None
        )
        dataset_dict = {
    ...
    def get_dataset(folder):
        ...
        url_with_instrument = "%s&instrument__id=%s" % (
            url,
            settings.general.instrument.instrument_id,
        )
        response = requests.get(
            headers=settings.default_headers, url=url_with_instrument
        )
        if response.status_code == 400:
            logger.debug("MyTardis doesn't support filtering datasets by instrument")
