"""
tests/commands/scan/test_scan_email_exp_dataset.py
"""
import textwrap
...
def test_scan_email_exp_dataset(set_email_exp_dataset_config):
    ...
    with requests_mock.Mocker() as mocker:
        ...
        assert result.output == "%s\n" % textwrap.dedent(
            """
            Scanning tests/testdata/testdata-email-exp-dataset/ using the "Email / Experiment / Dataset" folder structure...

            Found user folder: testuser1@example.com
            Found user folder: testuser2@example.com

            Found 2 dataset folders in tests/testdata/testdata-email-exp-dataset/
